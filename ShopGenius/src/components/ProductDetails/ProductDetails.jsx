...
const ProductDetails = () => {
  ...
  const { data: product, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.find((d) => d.productId === id);
    },
  });
  ...
  const priceNow =
    product?.discount?.active && product?.salePrice ? product.salePrice : product?.price;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <nav className="text-sm text-gray-500 mb-4">
        <Link to={'/'}>
          <span className="capitalize hover:underline cursor-pointer">Home</span>
        </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className="grid grid-cols-[70px_1fr] gap-3">
            ...
            {images?.map((src, idx) => (
              <button
                ...
                className={`w-[70px] h-[70px] cursor-pointer border rounded overflow-hidden ${
                  imageIndex === idx ? 'ring-2 ring-blue-500' : 'border-gray-200'
                }`}
              >
          ...
          <div className="order-1 lg:order-2 bg-white border border-gray-200 rounded p-3 flex items-center justify-center">
            {currentImg ? (
              ...
              <div className="h-[420px] w-full grid place-items-center text-gray-400">No image</div>
            )}
          </div>
        </section>
        ...
        <section className="space-y-5">
              ...
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg
                    ...
                    viewBox="0 0 20 20"
                    className={`w-4 h-4 ${
                      i < (product.rating ?? 0) ? 'fill-yellow-400' : 'fill-gray-300'
                    }`}
                  >
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-500">All prices include taxes where applicable.</p>
          </div>
          ...
          <div className="space-y-1 text-sm">
            <p
              className={`${
                product.availability?.status === 'in_stock' && product.availability?.stock > 0
                  ? 'text-green-700'
                  : 'text-red-600'
              }`}
            >
            ...
            {product.seller && (
              <p className="text-gray-600">
                Sold by{' '}
                <span className="font-medium text-gray-800">{product?.seller?.storeName}</span> —{' '}
          ...
          <div className="grid grid-cols-2 gap-3 max-w-md pt-2">
            <button className="btn cursor-pointer col-span-2 sm:col-span-1 rounded-full bg-amber-400 hover:bg-amber-500 text-gray-900 font-medium px-6 py-3 transition shadow">
              Add to Cart
            </button>
